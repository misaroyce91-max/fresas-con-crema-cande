...
import { ArrowRight, Bike, CalendarDays, Gift, MapPin, Sparkles, Star } from 'lucide-react'
...
export default function Home() {
  ...
  const now = new Date()
  const products = config.products.filter(product => product.active && product.featured).sort((a, b) => a.sortOrder - b.sortOrder)
  ...
  return <main className="page">
    ...
    <section className="mt-10">
      ...
      <div className="hide-scroll -mx-4 mt-4 flex gap-4 overflow-x-auto px-4 pb-4">
        {products.map(product => <Link href="/menu" key={product.id} className="card min-w-[190px] overflow-hidden"><div className="relative h-36"><Image src={product.image} alt={product.name} fill className="object-cover" /></div><div className="p-4"><h3 className="font-extrabold">{product.name}</h3><p className="mt-1 text-sm text-zinc-500">Desde <strong className="text-cande-700">{money(product.prices['10 oz'])}</strong></p></div></Link>)}
      </div>
    </section>
  </main>
}
